...
public class PostulerChauffeurController implements Initializable {
    ...
    @FXML private ImageView imgPermis;
    @FXML private Label lblPermisStatus;

    private byte[] permisBytes = null;
    private String permisFileName = null;
    private String permisMime = null;
...
// Listener dynamique
    ...
    @FXML
    void onUploadPermis(ActionEvent event) {
        try {
            FileChooser fc = new FileChooser();
            fc.setTitle("Choisir une photo du permis");
            ...
            permisBytes = Files.readAllBytes(file.toPath());
            permisFileName = file.getName();

            String lower = permisFileName.toLowerCase();
            if (lower.endsWith(".png")) permisMime = "image/png";
            else if (lower.endsWith(".jpg") || lower.endsWith(".jpeg")) permisMime = "image/jpeg";
            else permisMime = "application/octet-stream";

            // Preview
            imgPermis.setImage(new Image(new ByteArrayInputStream(permisBytes)));
            lblPermisStatus.setText("Fichier: " + permisFileName);

        } catch (Exception e) {
            permisBytes = null;
            permisFileName = null;
            permisMime = null;
            showError("Impossible de lire l'image. Réessayez.");
    ...
    void envoyerCandidature(ActionEvent event) {
        ...
        try {
            ...
            permisFileName = null;
            permisMime = null;
            imgPermis.setImage(null);
            lblPermisStatus.setText("Aucun fichier sélectionné");
            tfMaladieDetails.clear();
            tfMaladieDetails.setVisible(false);
            lblMaladieDetails.setVisible(false);
